...
def statusOfToken():
    ...
    cursor.execute('select count(*) from tokens where token_timestamp = "%s" and doctor_location_id = "%d" and serial_no < "%d" and status in ("assigned")' % (tokenTimestamp, int(doctorLocationId), int(serialNo)))
    result = cursor.fetchone()
